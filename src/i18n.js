...
function getLocaleId_gen() {
  const localeId = _getLocaleId();
  return () => localeId;
}

export const getLocaleId = getLocaleId_gen();

export function t(i18nId) {
  ...
  if (url.includes("{{")) {
    ...
    if (matches) {
      matches.forEach((match, _) => {
        url = url.replace(
          match,
          imgI18n[localeId][match.replace("{{", "").replace("}}", "")],
        );
      });
    }
  }
